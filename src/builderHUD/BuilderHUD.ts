...
class BuilderHUD {
    ...
    constructor () {
        ...
        try {
            this.selectionPointerShape = new GLTFShape(this.srcPath+"pointer.glb")
        }
        catch (e){
    ...
    setupUI (){
        ...
        let imageAtlas = this.srcPath + "builder-hud-atlas-1024.png"
...
//            position:new Vector3(camera.position.x-(pSX+1)*pX, 0-(pSY+1)*pY, camera.position.z-(pSZ+1)*pZ),
       ...
       // t.rotate(Vector3.Up(), 180) // not needed to get the model facing "north" when the cam rot is applied
        ...
        if (this.newEntityShape==null){
            ...
            try {
                this.newEntityShape = new GLTFShape(this.srcPath + "xyzLeftHand.glb")
            }
            catch (e){
